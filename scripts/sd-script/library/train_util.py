    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if tokenization_required:
                ...
                uncond_input_ids = [ids[0] for ids in self.tokenize_strategy.tokenize(uncond_caption)]  # remove batch dimension
                # if self.XTI_layers:
                #     caption_layer = []
            ...
            uncond_input_ids_list.append(uncond_input_ids)
            captions.append(caption)
        ...
        example["uncond_input_ids_list"] = none_or_stack_elements(uncond_input_ids_list, lambda x: x)
...
def get_timesteps(min_timestep: int, max_timestep: int, b_size: int, device: torch.device) -> torch.Tensor:
    if min_timestep < max_timestep:
        ...
        predefined_timesteps = [929, 910, 890, 868, 846, 823, 799, 773, 746, 717, 687, 655, 622, 586, 549, 510, 469, 427, 385, 341, 299, 256, 217, 179, 146, 116, 90, 68, 51, 36, 26, 18, 12, 8, 5, 3, 2, 1, 0]
        timesteps = torch.tensor(np.random.choice(predefined_timesteps, size=b_size), device="cpu")
    else:
